...
from PyQt6.QtGui import QIcon, QPixmap, QFont, QColor, QImage
from PyQt6.QtCore import Qt, QSize, QObjectCleanupHandler
from GuiConstants import color_palette, GuiViews, base_alcohols
from . import CenterQLabel
from typing import Callable
import math
import os
from pathlib import Path
from backend.datatypes import CocktailRecipe
from backend.cocktail_machine import CocktailMachineSingleton
import qrcode
current_directory = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
cocktail_machine = CocktailMachineSingleton.get_instance()
import io
...
class DrinkButton(QPushButton):
    def __init__(self, text=None):
        super(DrinkButton, self).__init__(text=text)
...
class DrinkList(QWidget):
    ...
    def main_setup(self):
        ...
        self.previous_button = ListNavigateButton(icon_path=current_directory+"/icons/return.png", on_click=self.load_previous)
        #self.previous_button.clicked.connect(self.load_previous)
        self.next_button = ListNavigateButton(icon_path=current_directory+"/icons/next-page.png", on_click=self.load_next)
    ...
    def setup_stacked_layout(self):
        ...
        self.horizontal_layouts = [QHBoxLayout() for _ in range(math.ceil(len(self.items)/ITEMS_TO_SHOW))]
...
class DetailedDrinkView(QWidget):
    ...
    def add_grid_spacer(self):
        widget = QSpacerItem(40,20,QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum)
        self.main_layout.addItem(widget,0,4,5,2)
    ...
    def add_icon(self):
        ...
        potential_path = current_directory + f"/icons/drinks/{self.cocktail.name.lower()}.png"
        icon_pixmap = QPixmap(potential_path if Path(potential_path).exists() else default_path)
        icon_pixmap = icon_pixmap.scaled(QSize(120,120))
        icon_label = CenterQLabel()
    ...
    def add_tags(self):
        ...
        horizontal_layouts = [QHBoxLayout() for _ in range(math.ceil(len(tags)/TAGS_PER_LINE))]
        ...
        self.main_layout.addLayout(vertical_layout, 4,1,3,3)
    
    def add_origin(self):
        origin_country = self.cocktail.origin_country
        if not origin_country or not Path(current_directory + f"icons/flags/{origin_country}"):
            return
        ...
        # MIGHT NOT NEEDED IN GRIDLAYOUT flag_pixmap = flag_pixmap.scaled()
        self.main_layout.addWidget(flag_label,6,1,3,3)
        
    def add_QR_code(self):
        qr_pixmap = generate_qr_code(self.cocktail.external_link,50)
        qr_pixmap = qr_pixmap.scaled(150,150)
    ...
    def add_title(self):
        ...
        self.main_layout.addWidget(title_label, 0,6,2,5)
    ...
    def add_description(self):
        ...
        font.setPointSize(14)
        description_label.setFont(font)
        self.main_layout.addWidget(description_label,2,6,3,4)
    ...
    def add_steps(self):
        ...
        self.main_layout.addWidget(QLabel(text),5,6,4,4)
        
    def add_submit_button(self):
        submit_button = QPushButton("Make Me A Drink!")
        def submit_drink_action():
            print("CREATING DRINK NOW!")
            cocktail_machine.pour_cocktail(self.cocktail.name+"TEST")
            
        submit_button.clicked.connect(submit_drink_action)
        self.main_layout.addWidget(submit_button,12,7,1,2)
        

class DrinkTag(QLabel):
    def __init__(self, tag_message: str):
        super(DrinkTag, self).__init__(text=tag_message)
