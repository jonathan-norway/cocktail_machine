...
from ..Components import SecondHeader, ModeMenuLayout, MenuModeCard, DrinkList, DrinkCard, MainMenu, DetailedDrinkView
...
class DrinkMenuView(MainMenu):
    ...
    def get_and_display_detailed_by_name(self, cocktail_name: str):
        drink_recipe = cocktail_machine.get_cocktail_recipe_by_name(cocktail_name)
    ...
    def get_and_display_drinks_by_base(self, base_alcohol: str):
        all_drinks_json_list = cocktail_machine.get_cocktail_recipes_by_base(base_alcohol)
        ...
        for cocktail_name, cocktail_data in all_drinks_json_list.items():
            potential_icon = current_directory + f"/icons/drinks/{'-'.join(cocktail_name.split())}.png"
    ...
    def inner_navigate(self, target_page: DrinkMenuModes):
        ...
        self.subheader.previous_button.update_nav(lambda: self.sub_menu_layout.setCurrentIndex(origin_page_index))
        ...
        for base_alcohol in base_alcohols:
            self.card_list.add_card(
                Card(
                    ...
                    on_click=lambda _, name=base_alcohol["name"]: self.get_and_display_drinks_by_base(name),
                )
            )
